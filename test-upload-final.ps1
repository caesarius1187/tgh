# Script final para probar la API de upload-file
...
# 3. Probar upload de foto con autenticación
Write-Host "`n🧪 Probando upload de foto con autenticación..." -ForegroundColor Yellow
try {
    ...
    $uploadResponse = Invoke-RestMethod -Uri "$baseUrl/api/upload-file" -Method POST -Headers $headers -Body $bodyLines
...
# 4. Probar upload sin autenticación (debe fallar)
Write-Host "`n🧪 Probando upload sin autenticación (debe fallar)..." -ForegroundColor Yellow
try {
    $boundary = [System.Guid]::NewGuid().ToString()
    $LF = "`r`n"
    ...
    $noAuthResponse = Invoke-RestMethod -Uri "$baseUrl/api/upload-file" -Method POST -Headers $headers -Body $bodyLines
    
    Write-Host "❌ No debería haber sido exitoso:" -ForegroundColor Red
    $noAuthResponse | ConvertTo-Json -Depth 3
...
# 5. Probar upload con tipo inválido
Write-Host "`n🧪 Probando upload con tipo inválido..." -ForegroundColor Yellow
try {
    $boundary = [System.Guid]::NewGuid().ToString()
    $LF = "`r`n"
    ...
    $invalidTypeResponse = Invoke-RestMethod -Uri "$baseUrl/api/upload-file" -Method POST -Headers $headers -Body $bodyLines
    
    Write-Host "❌ No debería haber sido exitoso:" -ForegroundColor Red
    $invalidTypeResponse | ConvertTo-Json -Depth 3
...
if (Test-Path "./public/uploads") {
    ...
    foreach ($file in $uploadedFiles) {
        ...
        Write-Host "  - $($file.Name) ($fileSize bytes)" -ForegroundColor Green
    }
} else {
...
Write-Host "`n🔍 Verificando datos en base de datos..." -ForegroundColor Yellow
try {
    $headers = @{
        "Authorization" = "Bearer $token"
    }
    
    $userDataResponse = Invoke-RestMethod -Uri "$baseUrl/api/user-data" -Method GET -Headers $headers
    
    Write-Host "✅ Datos de usuario:" -ForegroundColor Green
    $userDataResponse | ConvertTo-Json -Depth 3
    
} catch {
    Write-Host "❌ Error al obtener datos de usuario:" -ForegroundColor Red
    Write-Host $_.Exception.Message
}
